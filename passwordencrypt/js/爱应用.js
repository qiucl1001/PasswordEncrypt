...
if (s-->0) for (h < this.DB && (r = this[s] >> h) > 0 && (n = !0, e = int2char(r)); s >= 0;) i > h ? (r = (this[s] & (1 << h) - 1) << i - h, r |= this[--s] >> (h += this.DB - i)) : (r = this[s] >> (h -= i) & o, 0 >= h && (h += this.DB, --s)),
r > 0 && (n = !0),
n && (e += int2char(r));
return n ? e: "0"
}
...
h = this.s << o & this.DM;
for (r = this.t - 1; r >= 0; --r) i[r + s + 1] = this[r] >> n | h,
h = (this[r] & e) << o;
...
e = (1 << o) - 1;
i[0] = this[r] >> o;
for (var s = r + 1; s < this.t; ++s) i[s - r - 1] |= (this[s] & e) << n,
i[s - r] = this[s] >> o;
...
for (var r = 0,
o = 0,
n = Math.min(t.t, this.t); n > r;) o += this[r] - t[r],
i[r++] = o & this.DM,
o >>= this.DB;
if (t.t < this.t) {
    for (o -= t.s; r < this.t;) o += this[r],
    i[r++] = o & this.DM,
    o >>= this.DB;
    o += this.s
} else {
    for (o += this.s; r < t.t;) o -= t[r],
    i[r++] = o & this.DM,
    o >>= this.DB;
    o -= t.s
}
...
for (r = 0; r < i.t - 1; ++r) {
    var o = i.am(r, i[r], t, 2 * r, 0, 1); (t[r + i.t] += i.am(r + 1, 2 * i[r], t, 2 * r + 1, o, i.t - r - 1)) >= i.DV && (t[r + i.t] -= i.DV, t[r + i.t + 1] = 1)
}
...
if (! (o.t <= 0)) {
    var n = this.abs();
    if (n.t < o.t) return null != i && i.fromInt(0),
    ...
    if (0 != f) {
        ...
        null != i && (r.drShiftTo(a, i), s != h && BigInteger.ZERO.subTo(i, i)),
        r.t = a,
        r.clamp(),
        p > 0 && r.rShiftTo(p, r),
...
for (s = 0, t = 0; 256 > t; ++t) s = s + this.S[t] + i[t % i.length] & 255,
h = this.S[t],
this.S[t] = this.S[s],
this.S[s] = h;
this.i = 0,
...
for (var e = "",
r = 0; r + t < n.length;) e += n.substring(r, r + t) + "\n",
r += t;
...
function hex2b64(r) {
var t, a, n = "";
for (t = 0; t + 3 <= r.length; t += 3) a = parseInt(r.substring(t, t + 3), 16),
n += b64map.charAt(a >> 6) + b64map.charAt(63 & a);
for (t + 1 == r.length ? (a = parseInt(r.substring(t, t + 1), 16), n += b64map.charAt(a << 2)) : t + 2 == r.length && (a = parseInt(r.substring(t, t + 2), 16), n += b64map.charAt(a >> 2) + b64map.charAt((3 & a) << 4)); (3 & n.length) > 0;) n += b64padchar;
return n
}
function b64tohex(r) {
var t, a, n = "",
h = 0;
for (t = 0; t < r.length && r.charAt(t) != b64padchar; ++t) v = b64map.indexOf(r.charAt(t)),
v < 0 || (0 == h ? (n += int2char(v >> 2), a = 3 & v, h = 1) : 1 == h ? (n += int2char(a << 2 | v >> 4), a = 15 & v, h = 2) : 2 == h ? (n += int2char(a), n += int2char(v >> 2), a = 3 & v, h = 3) : (n += int2char(a << 2 | v >> 4), n += int2char(15 & v), h = 0));
...
return hex2b64(rsa.encrypt(pwd));
}